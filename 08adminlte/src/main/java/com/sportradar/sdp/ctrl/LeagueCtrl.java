...
import com.sportradar.sdp.dao.sdh.SdhLeagueDao;
...
public class LeagueCtrl {
	...
	@Autowired
	private SdhLeagueDao sdhLeagueDao;
	...
	public DataTablesOutput<League> findAll(@Valid DataTablesInput input) {
		DataTablesOutput<League> ds = this.sdhLeagueDao.findAll(input);
		return  ds;
	}
	...
	private League findById(Long id, Model model) {
		Optional<League> optionalRegion = this.sdhLeagueDao.findById(id);
